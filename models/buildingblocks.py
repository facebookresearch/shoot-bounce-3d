...
class SimpleFiLMLayer(nn.Module):
    def __init__(self, condition_size):
        super(SimpleFiLMLayer, self).__init__()
        self.gamma = nn.Linear(condition_size, 1)
        self.beta = nn.Linear(condition_size, 1)

    def forward(self, x, condition):
        gamma = self.gamma(condition).unsqueeze(1).unsqueeze(2)
        beta = self.beta(condition).unsqueeze(1).unsqueeze(2)
        return x * gamma + beta


class ComplexFiLMLayer(nn.Module):
    def __init__(self, num_features, condition_size):
        super(ComplexFiLMLayer, self).__init__()
        self.gamma = nn.Linear(condition_size, num_features)
        self.beta = nn.Linear(condition_size, num_features)
...
def create_conv(
    ...
    for i, char in enumerate(order):
        ...
        elif char == "f" and encoder is True:
            film = SimpleFiLMLayer(1)
            modules.append(("film", film))
        else:
            raise ValueError(
